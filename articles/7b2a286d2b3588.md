...
# つまづいたところ
# 下記コマンドを実行しても応答は無いが、少し放置して Ctrl + c で強制終了
# 強制終了したら次のコマンドへ
# （cmake の手前までの処理は動いている必要があるので実行必須）
python setup_env.py -md models/BitNet-b1.58-2B-4T -q i2_s

# cmake
cmake -B build -DBITNET_ARM_TL1=OFF -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DGGML_OPENMP=OFF -DCMAKE_BUILD_TYPE=Release
cmake --build build
...
そうです、最初から 2B の LLM にプログラミングさせるなど愚かな行為でした。何度も何度も生成を繰り返し、混沌の中から動くアプリケーションが生まれました。私が作り出したもののほとんどは、ただのガラクタ、デジタルのゴミに過ぎなかった。
